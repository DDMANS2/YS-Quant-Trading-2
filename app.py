"""
YS Quant Trading — 코스피·코스닥 종목 중 일봉 종가가 N일 이동평균선을 돌파한 종목을
자동 탐지하고, PER·PBR·ROE·PEG 가치 지표와 증권사 컨센서스 목표가까지 한 화면에서
확인하는 무료 한국 주식 스크리너.

- 코스피/코스닥 시가총액 상위 N개 또는 전체 스캔
- 일봉 종가 기준 N일 이동평균선 돌파 종목 검색 (병렬 처리 + 자동 재시도)
- 가치 지표: PER · PBR · ROE · PEG · EPS · BPS · 배당수익률 + 추정 PER
- 증권사 컨센서스: 평균 목표주가 + 투자의견 + 기준일 + 최근 리포트 5건
- 상세 차트: 캔들 + 이평(5/20/60/120) + 볼린저밴드 + 거래량 + MACD + RSI
- 인포그래픽 디자인 + 데스크톱/태블릿/모바일 반응형
